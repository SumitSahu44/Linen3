  ...
  const navLinks = [
    ...
    { name: 'Blog', href: '/blog' },
    {
      ...
      subLinks: [
        ...
        { name: 'Visit with Appointment', path: '/visit-appointment' },
        { name: 'Circular', path: '/circulars' },
        { name: 'Our Textile Associates', path: '/textile-associates' }
      ]
    },
  ];

  return (
    <nav className={`fixed w-full z-[100] transition-all duration-500 ${isScrolled ? 'top-0 py-2' : 'top-0 py-4'
      }`}>
      {/* Main Container: 
          - bg-slate-900/80 (Transparent Black) is always present.
          - backdrop-blur-md (Glass effect) blurs the content behind it.
      */}
      ...
      `}>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 cursor-pointer group">
            <div className="w-11 h-11 bg-white rounded-xl flex items-center justify-center font-black text-2xl text-blue-400 transition-transform group-hover:scale-110 shadow-lg shadow-white/10">
              P
            </div>
          ...
          <div className="hidden lg:flex items-center gap-7">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group py-2">
                <NavLink
                  to={link.href}
                  className={({ isActive }) => `text-[13px] font-bold uppercase tracking-widest flex items-center gap-1 transition-all active:scale-95 hover:text-blue-400 ${isActive && link.href !== '#' ? 'text-blue-400' : 'text-white/90'
                    }`}
                >
                  {link.name}
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <Link to="/trade-enquiry" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-wider transition-all shadow-lg shadow-blue-500/20 active:scale-95 text-center">
              Enquiry
            </Link>
          </div>
      ...
      <div className={`lg:hidden fixed inset-0 bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 z-[60] ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
        }`} onClick={() => setIsMobileMenuOpen(false)}>

        <div className={`absolute right-0 top-0 h-full w-[80%] bg-slate-900 shadow-2xl transition-transform duration-500 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          ...
          <div className="p-8 flex flex-col h-full overflow-y-auto">
            ...
            <div className="mt-10 space-y-3 pb-8">
              <Link to="/trade-enquiry" className="block w-full py-4 bg-blue-600 text-white text-center rounded-2xl font-black uppercase tracking-widest text-xs">
                Trade Enquiry
              </Link>
            </div>
          </div>
